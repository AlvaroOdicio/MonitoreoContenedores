...
// --- Define tus umbrales ---
const UMBRAL_CRITICO = 90; // (90% o más) Alerta Roja
...
export default function Home() {
  ...
  const [isResolving, setIsResolving] = useState(false); // Estado para el botón de resolver
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Filtramos solo los que están operativos
  ...
  const handleResolveReport = async (reportGuid: string) => {
    setIsResolving(true); // Activa el loading del botón
    try {
      await resolveMalfunctionReport(reportGuid);
      // Éxito: Volvemos a cargar todos los datos para refrescar la lista
      await loadDashboardData();
    } catch (err: any) {
      console.error("Error al resolver reporte:", err);
      alert(`Error: ${err.message}`);
    } finally {
      setIsResolving(false); // Desactiva el loading del botón
    }
  };
  ...
  useEffect(() => {
    ...
    // 2. Función para buscar direcciones
  ...
  // --- CÁLCULO DE MÉTRICAS (en el render) ---
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {/* Recent Activity and Alerts - DINÁMICO */}
          ...
          {/* --- 👇 [NUEVA SECCIÓN] ALERTAS CIUDADANOS --- */}
